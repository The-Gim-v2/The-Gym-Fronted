...
<script setup>
// CAMBIO: se agrega "computed", que hacía falta para declarar `presetActivo`.
// El template ya lo usaba (:class="{ 'is-active': presetActivo === key }")
// pero nunca existía en el script, así que esa comparación siempre daba
// `undefined === key` (false) y ningún preset se marcaba como activo.
...
const translations = {
  es: {
    ...
    toastRecepcionSaved: "Configuración de Recepcion guardada globalmente.",
  ...
  en: {
    ...
    toastRecepcionSaved: "Recepcion configuration saved globally.",
...
// CAMBIO: `t` depende de `settings.idioma`, así que sigue definiéndose aquí,
// pero ahora `settings` se declara más abajo, después de todos sus
// dependientes (ver el bloque "CAMBIO: orden de declaración" más adelante).
// Como `t` es una función (no se ejecuta hasta que se llama), esto no da
// problema de "usada antes de declararse".
const t = (key) => {
  return translations[settings.idioma]?.[key] || translations['es'][key] || key;
};

// ============================================================
// CAMBIO CLAVE #1: safeJsonParse
// Antes se usaba JSON.parse(localStorage.getItem(...)) directo.
// Si en ese celular la llave guardada no era JSON válido (dato viejo,
// corrupto, o simplemente no existía y JSON.parse recibía null/"undefined"),
// JSON.parse lanzaba una excepción DENTRO del <script setup>. Eso tumba el
// montaje de TODO el componente sin mostrar nada en pantalla — exactamente
// el "en un celular sí entra, en otro no muestra nada". Con este helper,
// si el dato está corrupto simplemente se ignora y se usa el valor por
// defecto, en vez de romper la vista completa.
// ============================================================
...
// ============================================================
// CAMBIO CLAVE #2: orden de declaración
// ROLE_KEY, claveColores, claveRadius, claveDensidad y defaultColors ahora
// se declaran ANTES de `settings`, porque `settings` los usa en su propia
// inicialización. En JS, un `const` no existe hasta que su línea se ejecuta
// (temporal dead zone); llamarlo antes lanza "Cannot access before
// initialization" y —igual que el punto anterior— tumba el componente
// completo. Antes estaban declarados DESPUÉS de `settings`, lo cual rompía
// el montaje siempre (no solo en un celular).
// ============================================================
const ROLE_KEY = 'user_role'; // misma llave que usa el router y App.vue

// Namespacing por rol: cada rol (owner/member/recepcion) guarda SU PROPIO
// tema, así aplicar un preset en Recepción ya no le cambia el color a Owner
// ni a Member (antes todos escribían/leían la misma llave global "app-colors").
const claveColores = () => {
  const rol = (localStorage.getItem(ROLE_KEY) || '').toLowerCase();
  return `app-colors-${rol}`; // ej: app-colors-owner, app-colors-member, app-colors-recepcion
};
const claveRadius = () => {
  const rol = (localStorage.getItem(ROLE_KEY) || '').toLowerCase();
...
// CAMBIO: presetActivo faltaba por completo en el script (el template ya lo
// usaba). Se agrega como computed: compara los colores actuales contra cada
// preset y devuelve la llave del que coincide, para marcar su tarjeta con el
// check ✓ y el resaltado "is-active".
...
const guardarCambios = async () => {
  try {
    const userRole = localStorage.getItem('user_role') || 'Recepcion';
